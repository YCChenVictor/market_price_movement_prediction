...
# tfrecords
def read_and_decode(filename, batch_size, num_of_epoches, periods, n_instruments):
    ...
    input_tensor = tf.reshape(
        input_tensor, [periods, n_instruments + 1, n_instruments + 1]
    )
...
def neural_network_CNN_RNN(x, time_step, cnn=neural_network_CNN()):
    ...
    model_output = keras.layers.TimeDistributed(
        cnn, input_shape=(time_step, 31, 31, 1)
    )(model_input)
