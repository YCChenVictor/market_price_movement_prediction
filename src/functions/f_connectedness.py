...
def generalized_variance_decomp(m, coef, sigma_hat, h=1):
    ...
    num = []
    decomp = []
    den_fill = np.linalg.multi_dot(
        (m_i, theta_value[0], theta_value[0].T, m_i[np.newaxis].T)
    )
    den.append(den_fill)
    ...
    for l in range(1, h):
        den_fill = den[l - 1] + (
            np.linalg.multi_dot(
                (m_i, theta_value[l], theta_value[l].T, m_i[np.newaxis].T)
            )
        )
        den.append(den_fill)
        num_fill = np.square(np.linalg.multi_dot((m_i, psi[l], sigma_hat))) + num[l - 1]
        num.append(num_fill)
        decomp.append(num_fill * inv_sigma2 / den_fill)
    return decomp
...
class Connectedness:
    ...
    def f_full_connectedness(self, h=1):
        ...
        up = np.concatenate(
            (np.matrix(connectedness), np.matrix(from_other).transpose()), axis=1
        )
    ...
    def rename_table(self, names):
        ...
        full_connectedness.rename(
            index=dict(zip(full_connectedness.index, names)), inplace=True
        )
